# Bundled Help / Bachao keyword model
...
SafeCity uses the 320 ms `chunk-16` model for the higher-accuracy mobile tradeoff. `HELP` uses an English CMU-phone sequence. `BACHAO` combines CMU-phone and pinyin-token acoustic paths to cover Hindi and Indian-English pronunciations without sending audio to a speech service. The `2.0` score and `0.20` threshold are initial values validated against synthetic positive and negative speech; they are not production accuracy claims.

The bundled model removes any dependency on an installed operating-system language pack. It still requires representative physical-device testing across accents, microphones, noise levels, playback audio, and low-end CPUs before production accuracy or battery claims can be made.
